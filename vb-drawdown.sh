...
BASE_URL=$(jq -r '.vb.base_url' "$CONFIG_FILE")
LOC_ACCOUNT_NO=$(jq -r '.vb.loc_account_no' "$CONFIG_FILE")
DISBURSE_AMOUNT=$(jq -r '.vb.disburse_amount' "$CONFIG_FILE")
TO_ACCOUNT_NO=$(jq -r '.vb.to_account_no' "$CONFIG_FILE")
PRODUCT_MARKET_CODE=$(jq -r '.vb.product_market_code' "$CONFIG_FILE")
SELECTED_PLAN_ID=$(jq -r '.vb.selected_plan_id' "$CONFIG_FILE")
CCD_ID=$(jq -r '.vb.ccd_id' "$CONFIG_FILE")

# Set the drawdown type: "Saveing" or "bill"
DRAWDOWN_TYPE=$(jq -r '.vb.drawdown_type' "$CONFIG_FILE")
...
delay_and_log() {
    ...
    echo -e "${YELLOW}Adding 60-second delay before API call...${NC}"
...
echo -e "${YELLOW}Calling POST https://intgw-dlp-sit.core-bank.tripperpix.com/dcb/lending/v1/drawdown/installmentation${NC}"
...
response=$(curl --location 'https://intgw-dlp-sit.core-bank.tripperpix.com/dcb/lending/v1/drawdown/installmentation' \
--header 'x-devops-src: bib' \
--header 'x-devops-dest: vb-dlp' \
--header 'x-devops-key: tS19zj2II4CKO0w13UnVGavXQp0KO83u' \
--header 'x-channel-id: bib' \
--header "x-request-id: ${REQUEST_ID_1}" \
--header "x-traceparent: ${TRACE_PARENT_UUID}" \
--header 'Content-Type: application/json' \
--data "$REQUEST_BODY_1" -k)
...
echo -e "${YELLOW}Calling POST https://intgw-dlp-sit.core-bank.tripperpix.com/dcb/lending/v1/drawdown/submit-to-saving${NC}"
...
response2=$(curl --location 'https://intgw-dlp-sit.core-bank.tripperpix.com/dcb/lending/v1/drawdown/submit-to-saving' \
--header 'x-devops-src: bib' \
--header 'x-devops-dest: vb-dlp' \
--header 'x-devops-key: tS19zj2II4CKO0w13UnVGavXQp0KO83u' \
--header 'x-channel-id: bib' \
...
# Step 3: Confirm Drawdown
if [ "$DRAWDOWN_TYPE" = "Saveing" ]; then
    echo -e "${GREEN}===== Step 3: Confirm to Saving =====${NC}"
    echo -e "${YELLOW}Calling POST https://intgw-dlp-sit.core-bank.tripperpix.com/dcb/lending/v1/drawdown/confirm-to-saving${NC}"
    ...
    response3=$(curl --location 'https://intgw-dlp-sit.core-bank.tripperpix.com/dcb/lending/v1/drawdown/confirm-to-saving' \
    --header 'x-devops-src: bib' \
    --header 'x-devops-dest: vb-dlp' \
    --header 'x-devops-key: tS19zj2II4CKO0w13UnVGavXQp0KO83u' \
    --header 'x-channel-id: VB' \
    --header "x-request-id: ${REQUEST_ID_3}" \
    --header "x-traceparent: ${TRACE_PARENT_UUID}" \
    --header 'Content-Type: application/json' \
    --data "$REQUEST_BODY_3" -k)
elif [ "$DRAWDOWN_TYPE" = "bill" ]; then
    echo -e "${GREEN}===== Step 3: Confirm to Biller =====${NC}"
    echo -e "${YELLOW}Calling POST https://intgw-dlp-sit.core-bank.tripperpix.com/dcb/lending/v1/drawdown/confirm-to-biller${NC}"
    REQUEST_ID_3=$(generate_request_id)
    echo -e "${YELLOW}Using request ID: ${REQUEST_ID_3}${NC}"
    ...
    # Add delay and log request body
    delay_and_log "Step 3: Confirm to Biller" "$REQUEST_BODY_3"

    response3=$(curl --location 'https://intgw-dlp-sit.core-bank.tripperpix.com/dcb/lending/v1/drawdown/confirm-to-biller' \
    --header 'x-devops-src: bib' \
    --header 'x-devops-dest: vb-dlp' \
    --header 'x-devops-key: tS19zj2II4CKO0w13UnVGavXQp0KO83u' \
    --header 'x-channel-id: VB' \
    ...
    echo -e "${RED}Error: Invalid DRAWDOWN_TYPE value: ${DRAWDOWN_TYPE}. Must be 'Saveing' or 'bill'.${NC}"
    exit 1
fi
