...
/**
 * Connect to a PostgreSQL database for KTB
...
async function connectToDatabase(database) {
  ...
  const client = new Client({
    host: config.ktb.db_host || process.env.DB_HOST,
    port: config.ktb.db_port || process.env.DB_PORT,
    database: database,
    user: config.ktb.db_user || process.env.DB_USER,
    password: config.ktb.db_password || process.env.DB_PASSWORD,
    schema: 'public'
  });

  try {
    await client.connect();
    console.log(colors.green(`Connected to KTB ${database} database`));
    return client;
  } catch (error) {
    console.log(colors.red(`Error connecting to KTB ${database} database: ${error.message}`));
    throw error;
  }
}
...
async function updateLoanSmartContract(client, supervisorContractId, locSmartContractId, drawdownSmartContractId) {
  try {
    ...
    console.log(colors.green(`Updated ${result.rowCount} rows in KTB public.loan_smart_contract`));
    return result.rowCount;
  } catch (error) {
    console.log(colors.red(`Error updating KTB loan_smart_contract: ${error.message}`));
    throw error;
  }
}
...
async function updateProcLoanAccount(client, supervisorContractId, locSmartContractId, drawdownSmartContractId) {
  try {
    ...
    const result = await client.query(query);
    console.log(colors.green(`Updated ${result.rowCount} rows in KTB public.proc_loan_account`));
    return result.rowCount;
  } catch (error) {
    console.log(colors.red(`Error updating KTB proc_loan_account: ${error.message}`));
    throw error;
  }
}
